  ...
  module Admin
    class FavoriteProductsController < Spree::Admin::BaseController
      ...
      def index
        ...
        @favorite_products = @search.records.includes(master: :images).joins(:master).order_by_favorite_users_count(sort_in_ascending_users_count?).page(params[:page]).per(Spree::Config.favorite_products_per_page)
      end
